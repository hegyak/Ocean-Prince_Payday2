	...
	function MusicManager:current_track_string()
		...
		elseif ((Music:get_list() == 0 or Music:get_list() == 7 or Music:get_list() == 8 or Music:get_list() == 9 or Music:get_list() == 10 or Music:get_list() == 11) and Music.pressed == false) or (Music.autowave == true and Music.pressed == false and Music:EventLock() == false) then
			...
			if music_style == "heist" then
				local tempfix = Global.music_manager and Global.music_manager.current_track
				return utf8.to_upper(managers.localization:text("menu_jukebox_" .. tempfix))
			end
...
function Music:Shuffle()
	if Music.settings.mcontrol_shuffle == true then
		MenuCallbackHandler.music_call_random_func()
	else
		MenuCallbackHandler.music_call_next_func()
	end
end
...
function Music:Stage()
	...
	function LevelsTweakData:get_music_event(stage)
		...
		if music_id == "ghost" then
			...
			Music.block_lv = true
			return nil
		elseif self:get_music_style_from_level_data(level_data) ~= "heist" then
			Music.autowave = true
			if stage == "control" and Music.settings.mcontrol_switchonwave == true then
				Music:Shuffle()
			end
...
function Music:Call(type, song, mode, song_change)
	...
	if type == 1 then
		...
		elseif not managers.music.attempt_play or not managers.music:attempt_play(song) then
			msg_menu_title = managers.localization:text("menu_jukebox_" .. song)
...
Hooks:Add("MenuManagerInitialize", "MenuManagerInitialize_MusicManagerInit", function(menu_manager)
	---- ### MUSIC NEXT BUTTON ###
	MenuCallbackHandler.music_call_next_func = function(self)
	...
	MenuCallbackHandler.music_call_prev_func = function(self)
		if Music:Chat() == false and Music:isRestrictMap() == false then
			if Music:get_list() == 0 then
				Music.juke_track = Music.juke_track - 1
	...
	---- ### MUSIC RANDOM BUTTON ###
	MenuCallbackHandler.music_call_random_func = function(self)
		if Music:Chat() == false and Music:isRestrictMap() == false then
			if Music:get_list() == 0 then
	...
	MenuCallbackHandler.music_call_list_func = function(self)
		if Utils:IsInHeist() then 
			Music.pressed = true 
		end
		...
		if Music:Chat() == false and Music:isRestrictMap() == false then
			...
			if Music:get_list_state() == false then
				return MenuCallbackHandler.music_call_list_func()
			end
			if Music:get_list() == 0 then
	...
	---- ### MUSIC MODE BUTTON ###
	MenuCallbackHandler.music_call_mode_func = function(self)
		if Music:Chat() == false and Music:isRestrictMap() == false then
	...
	MenuCallbackHandler.music_call_stop_func = function(self)
	...
	---- ### DIRECT MENU ###
	MenuCallbackHandler.music_call_direct_func = function(self)
